...
```bash
curl -s -H "Authorization: token $TOKEN" "https://github.com/trending/用户请求" | python3 -c "
import sys, re
...
如果 token 为空，回退到 Codex 网页工具 抓取 `https://github.com/trending/用户请求`。
...
- 如果用户指定了语言（`用户请求`），在标题中注明（如 "GitHub 热榜 — Python"）
- 用中文输出所有非代码内容
