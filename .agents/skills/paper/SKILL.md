...
```bash
# 首次：安装依赖
pip install -r .agents/skills/paper/requirements.txt

# 同步 Zotero 新论文并建/更新索引（增量；--full 全量重扫+重建，--limit N 调试）
python3 .agents/skills/paper/paper.py sync

# 问答：带问题=单次；省略=进交互式 REPL
python3 .agents/skills/paper/paper.py ask "我的库里关于 MoE 路由有哪些工作?"
python3 .agents/skills/paper/paper.py ask

# 查看缓存/索引状态
python3 .agents/skills/paper/paper.py status
```

## 作为 `/paper` 被调用时

`用户请求` 第一个词是子命令（`sync` / `ask` / `status`），缺省按 `ask` 处理：
