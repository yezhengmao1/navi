...
# 2. 推送
python3 .agents/skills/hiboard/push.py --data /tmp/task.json

# 先看 payload 不实际发送
python3 .agents/skills/hiboard/push.py --data /tmp/task.json --dry-run
```

也可不建文件、直接从别处拿正文：

```bash
python3 .agents/skills/hiboard/push.py --name "简报" --content report.md
echo "# 正文" | python3 .agents/skills/hiboard/push.py --name "简报" --content -
```
