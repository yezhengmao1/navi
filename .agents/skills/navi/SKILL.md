...
python3 .agents/skills/navi/navi.py sync

# 真镜像：同时删除远端多余文件
python3 .agents/skills/navi/navi.py sync --delete

# 预演（不实际传输，只看会动哪些文件）
python3 .agents/skills/navi/navi.py sync --dry-run

# 换机恢复：WebDAV → 本地 ~/.navi
python3 .agents/skills/navi/navi.py pull

# 看本地与远端差异
python3 .agents/skills/navi/navi.py status
```

## 作为 `/navi` 被调用时

`用户请求` 第一个词是子命令（`sync` / `pull` / `status`），缺省按 `sync` 处理：
