...
- 工具目录：`.agents/skills/swanlab/tools/`（凭证读 `~/.navi/config.toml` 的 `[swanlab]`）
- 指标说明：优先 `~/.navi/swanlab-metrics.md`，否则 `.agents/skills/swanlab/metrics.example.md`（取其「期望趋势/异常信号」作研判依据）。
...
- **参数为空**：用 `list_projects.py` 找 `count.runningExps>0` 的项目，再 `list_experiments.py` 取 `state==RUNNING` 的实验；多个用 `Codex 用户输入工具` 选。
...
1. 取最近窗口的点（无状态，用 `--tail`）：
   ```
   python3 .agents/skills/swanlab/tools/get_metrics.py --path <path> --keys <核心指标> --tail 50
   ```
