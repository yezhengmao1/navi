...
设计原则（沿用 navi 惯例）：
- **最小职责**：所有读写走唯一脚本 `.agents/skills/server/tools/server_config.py`（只增删查 `[servers.*]`，不做连接测试），本 skill 只负责**收集信息 + 编排**。
- **绝不重写整个配置**：脚本对其它段（`[github]`/`[swanlab]`/`[siyuan]`）做文本级保护，原样不动。

## 子命令

`用户请求` 第一个词是子命令，缺省按 `add` 处理：
...
```bash
python3 .agents/skills/server/tools/server_config.py add \
...
```bash
python3 .agents/skills/server/tools/server_config.py list
```
...
从 `用户请求` 取 `<name>`；没给就先 `list` 让用户挑。删除前**确认一次**，再执行：

```bash
python3 .agents/skills/server/tools/server_config.py remove --name gpu01
```
