...
```bash
python3 .claude/skills/feishu/push.py --key <KEY> --content report.md
echo "# 正文" | python3 .claude/skills/feishu/push.py --key <KEY> --content -
```

带标题 → 自动升级为 interactive 卡片（正文按 lark_md 渲染 markdown：加粗、列表、链接等）：

```bash
python3 .claude/skills/feishu/push.py --key <KEY> --title "每日简报" --content report.md
```
...
EOF
python3 .claude/skills/feishu/push.py --key <KEY> --data /tmp/task.json
python3 .claude/skills/feishu/push.py --key <KEY> --data /tmp/task.json --dry-run  # 只看 payload
```
