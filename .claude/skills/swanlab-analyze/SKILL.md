...
- 工具目录：`.claude/skills/swanlab/tools/`（`list_projects.py` / `list_experiments.py` / `get_summary.py` / `get_metrics.py`，均输出 JSON，凭证读 `~/.navi/config.toml` 的 `[swanlab]`）
- 指标说明：优先 `~/.navi/swanlab-metrics.md`，否则 `.claude/skills/swanlab/metrics.example.md`。**用户唯一需维护的文件**，描述每个指标的含义/期望趋势/健康范围/异常信号。
...
- **参数为空**：
  - 配置有默认 `project` → `list_experiments.py --project <默认>` 列出来；多个用 `AskUserQuestion` 选，单个直接用。
  - 无默认 project → 先 `list_projects.py` 列项目（可看 `count.runningExps` 找在跑的），`AskUserQuestion` 选项目再列实验。
