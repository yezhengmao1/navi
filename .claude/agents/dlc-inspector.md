...
GPU=0 的辅助任务（convert-ckpt 等）已默认过滤——不是训练任务，不巡检。若 `jobs` 为空且下面排队也为空，直接用 hiboard skill 推一条「当前无 Running / 排队训练任务」并结束。

### 1b. 列出排队中（Queuing）的 GPU 任务

再用 `Skill` 工具调用 `dlc` skill，参数 `list --json --status Queuing`。同样 GPU=0 已默认过滤，**只汇报占 GPU 的排队任务**。这些任务还没跑、没有日志，**不做健康研判**，只在报告里单独列一个「排队等待」区：每个一行带 属主 / 卡数 / 已等待时长（`duration`，从创建时间算起）/ 任务名 / 工作空间，并给出**排队合计卡数**。排队为空则省掉该区。
...
- 顶部一行概况：地域、Running 任务数、合计卡数，三级各几个（如「🔴1 ⚠️3 ✅7」），外加**排队 N 个 / M 卡**。
...
- `## ⏳ 排队等待`：来自第 1b 步的 Queuing GPU 任务，**逐个列全**，每个一行带 属主 / 卡数 / 已等待时长 / 任务名 / 工作空间，末尾给排队合计卡数。排队任务不做健康判级（还没日志）。排队为空则省掉该区。
- 任务名保留英文原文，属主用 `owner`（真人名）。某级为空就省掉该分区标题；全部正常时 🔴/⚠️ 区可不出现，但 ✅ 区仍要逐个列全。

自检：报告里 🔴+⚠️+✅ 的任务条数之和 == 第 1 步 `jobs` 总数（Running）；⏳ 排队区条数 == 第 1b 步 Queuing 任务数。漏了就补上再推。
...
2. **飞书群**：用 `Skill` 工具调用 `feishu` skill，同一份报告作为正文，`--title` 用「DLC 训练任务巡检」（带标题会以 markdown 卡片呈现，分级 emoji / 表格更好看）。**飞书 KEY 由调用方在触发时以参数给出（`--key <KEY>`），本 agent 只把它透传给 feishu skill，不写进配置、不落盘持久化、不硬编码 webhook**。调用时没给 KEY，就跳过飞书渠道（只推负一屏），并在小结里说明「未提供飞书 KEY，跳过飞书推送」。

两渠道**各自独立判成败**：分别读各自 skill 的返回，成功记「✅ 已推送」，失败把该渠道的错误码原样带出（如负一屏授权码无效 / 开关未开；飞书签名校验或关键词未命中），**一个失败不影响另一个**，不谎报成功。某渠道未配置或未提供 KEY（feishu 缺 `--key` / hiboard 缺 auth_code），如实说明「该渠道未配置，已跳过」，另一渠道照常推。
